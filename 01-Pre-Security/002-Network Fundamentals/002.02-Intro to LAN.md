...
| Concept | Key Role | Main Advantage | Main Disadvantage |
| --- | --- | --- | --- |
| **Star Topology** | Devices via central switch | Reliable, scalable | Central device failure |
| **Bus Topology** | Shared backbone cable | Low cost | Single cable failure, bottlenecks |
| **Ring Topology** | Devices in a loop | Simple, no collisions | Full network failure on break |
| **Switch** | Connects LAN devices | Efficient data transfer | Cost increases with size |
| **Router** | Connects networks | Enables inter-network communication | Complex configuration |
| **Subnetting** | Divides networks | Security & efficiency | Complex planning |
| **ARP** | Maps IP ↔ MAC | Enables local communication | Cache can become outdated |
| **DHCP** | Auto-assigns IPs | Simplifies management | Dependency on DHCP server |