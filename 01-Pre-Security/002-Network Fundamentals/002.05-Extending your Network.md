...
- **Switch**: Connects devices in one network →
- **Router**: Connects multiple networks →
- **Port Forwarding**: Allows external access to internal services →
- **Firewall**: Controls what traffic passes through →
- **VPN**: Secures connections between separate networks over the Internet.