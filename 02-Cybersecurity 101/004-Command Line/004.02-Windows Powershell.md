...
- Think in **objects**; use the **pipeline** to compose tasks.
- Learn the “discoverability trio”: `Get-Command`, `Get-Help`, `Get-Alias`.
- Master a small core set of cmdlets (`Get/Set/New/Remove/Copy/Move-Item`, `Get-ChildItem`, `Where-Object`, `Sort-Object`, `Select-Object`, `Select-String`) — they cover most daily needs.
- For administration, incident response, and automation, add `Get-Process`, `Get-Service`, `Get-*Net*`, `Get-FileHash`, and `Invoke-Command`.